...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(`https://ficback.onrender.com/api/v1/get-all-orders`, { headers })
  ...
  const submitChanges=async (i) => {
    ...
    const response = await axios.put(`https://ficback.onrender.com/api/v1/update-status/${id}`, Values,{ headers });
                ...
                <div className="w- [30%] md:w-[16%]">
                  <h1 className="font-semibold ">
                    <button className="hover:scale-105 transition-all duration-300" onClick={()=>setOptions(i)}>
                      {items.stauts == "Order placed" ? (
                        <div className="text-yellow-500">{items.stauts}</div>
                      ) : items.stauts == "Canceled" ? (
                        <div className=" text-red-500">{items.stauts}</div>
                      ) : (
                        <div className="text-green-500">{items.stauts}</div>
                      )}
                    </button>
